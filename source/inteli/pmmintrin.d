...
unittest
{
    ...
    assert( _mm_addsub_ps(v1,v2) == _mm_setr_ps(0.0f, 4.0f, 0.0f, 8.0f) );
}
...
unittest
{
    ...
    assert( _mm_hadd_pd(A, B) ==_mm_setr_pd(3.5, 3.0) );
}
...
unittest
{
    ...
    assert(_mm_hadd_ps(A, B)==_mm_setr_ps(3.0f, 8.0f, 3.5f, 7.5f));
}
...
unittest
{
    auto A =_mm_setr_pd(1.5, 2.0);
    auto B =_mm_setr_pd(1.0, 2.0);
    assert( _mm_hsub_pd(A, B) ==_mm_setr_pd(-0.5, -1.0) );
}
...
unittest
{
    __m128 A =_mm_setr_ps(1.0f, 2.0f, 3.0f, 5.0f);
    __m128 B =_mm_setr_ps(1.5f, 2.0f, 3.5f, 4.0f);
    assert(_mm_hsub_ps(A, B)==_mm_setr_ps(-1.0f, -2.0f, -0.5f, -0.5f));
}
...
unittest
{
    ...
    assert(_mm_movedup_pd(A) == _mm_set_pd(7.0, 7.0));
}

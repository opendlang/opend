...
version (Shared)
{
    ...
    @property ref HashTab!(void*, DSO*) _handleToDSO() @system @nogc nothrow { __gshared HashTab!(void*, DSO*) x; return x; }
    ...
    if (*data._slot is null)
    {
        ...
        *data._slot = pdso; // store backlink in library record
        ...
        version (Windows)
        {
            ...
            pdso._moduleGroup = ModuleGroup(header.getModuleInfos());
        }
        else
        {
            version (LDC)
...
@nogc nothrow:
    ...
    DSO* dsoForHandle(void* handle) @system
    {
        DSO* pdso;
    ...
    void setDSOForHandle(DSO* pdso, void* handle) @system
    {
        _handleToDSOMutex.lock_nothrow();
...
{
    static if (SharedELF) void scanSegments(const scope ref SharedObject object, DSO* pdso) nothrow @nogc @system
    {
        foreach (ref phdr; object)
        {
            switch (phdr.p_type)

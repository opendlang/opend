...
//===----------------------------------------------------------------------===//
//
// The LDC version of this code is based on LLVM 16 and modified such that it
// compiles with earlier LLVM versions.
//
//===----------------------------------------------------------------------===//
...
inline bool isSpace_ldc(char C) {
  return C == ' ' || C == '\f' || C == '\n' || C == '\r' || C == '\t' ||
         C == '\v';
}
...
[[nodiscard]] StringRef detectEOL(StringRef const &str) {
...
static int handle(MemoryBuffer &inputBuf, StringRef input) {
  ...
  for (line_iterator i(inputBuf, /*SkipBlanks=*/false, '\0'); !i.is_at_eof();) {
    ...
    if (isSpace_ldc(partName.front()) || isSpace_ldc(partName.back())) {
  ...
  for (auto &keyValue : partToBegin) {
    ...
    using namespace std;
    auto f = make_unique<ToolOutputFile>(partPath.str(), ec,
                                              llvm::sys::fs::OF_None);
    if (!f)
      fatal(input, ec.message());

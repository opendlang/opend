...
private:
  ...
  void addContext(bool objcdirectcall) {
    ...
    if (dfnval && (dfnval->func->ident == Id::ensure ||
      ...
      if(objcdirectcall) {
        // FIXME: dmd's objc_glue.d setupMethodCall shows how to do it
        // need the class def added too; the arg is a pointer to a struct
        // with the existing this and the class reference
        error(loc, "Super calls for objc not yet implemented in ldc");
        fatal();
      } else {

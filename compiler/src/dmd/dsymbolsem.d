                        ...
                        else if (ie.type)
                        {
                            ...
                            else if (auto ne = ie.isNewExp())
                            {
                                ...
                                else if (ne.arguments && ne.arguments.length == 1)
                                {
                                    ...
                                    {
                                        tsa.dim = new IntegerExp(loc, intExp.getInteger, Type.tsize_t);
                                    }
                                    else
                                    {
                                        tsa.dim = new IntegerExp(loc, 1, Type.tsize_t);
            ...
            if (auto d = tsa.dim)
            {
                if (auto ide = d.isIdentifierExp())
                {
                    if (ide.ident == Id.dollar)
                    {
                        ...
                        {
                            Expression ie = dsym._init.initializerToExpression(null, cast(bool)(sc.flags & SCOPE.Cfile));
...
bool evalPragmaExplicitGc(Loc loc, Scope* sc, Expressions* args){
    ...
    if (!e.type)
    {
        ...
        e = e.toBoolean(sc);
        if (e.isErrorExp())
            .error(loc, "pragma(`explicit_gc`, `true` or `false`) expected, not `%s`", (*args)[0].toChars());
        (*args)[0] = e;
    }
...
bool evalPragmaForceForeachRewrite(Loc loc, Scope* sc, Expressions* args){
    if (!args || args.length == 0)
        return true;
    if (args && args.length > 1)
    {
        .error(loc, "one boolean expression expected for `pragma(forceClosureRewrite)`, not %llu", cast(ulong) args.length);
        args.setDim(1);
        (*args)[0] = ErrorExp.get();
    }
    ...
    if (!e.type)
    {
        ...
        e = e.toBoolean(sc);
        if (e.isErrorExp())
            .error(loc, "pragma(`forceClosureRewrite`, `true` or `false`) expected, not `%s`", (*args)[0].toChars());
        (*args)[0] = e;
    }
